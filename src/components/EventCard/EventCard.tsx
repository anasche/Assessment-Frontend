...
const EventCard: React.FC<EventCardProps> = ({
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
        ...
        <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1 w-full md:w-auto">
          ...
          <div className="flex flex-col gap-0.5 min-w-0">
            <span className="font-sans font-normal text-[10px] lg:text-[12px] uppercase tracking-[0.1em] text-white/40">
              Event Country
            </span>
  ...
  if (slug) {
    return (
      <Link to={`/races/${slug}`} className="group block h-full hover:scale-[1.02] transition-transform duration-500">
        {content}
      </Link>
    );
  }
